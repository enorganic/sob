...
class Version:
    ...
    def _update_version_parameters_from_string(
        self,
        version_: str
    ) -> None:
        specification: str = ''
        for version_specifier in version_.split(','):
            ...
            operator_str: str
            variable_name: str
            for operator_str, variable_name in (
                _REPR_OPERATORS_VERSION_PROPERTIES
            ):
                if operator_str in version_specifier:
                    spec, version_number_string = (
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        for compare_property_name, compare_function in (
        ...
        for property_name, repr_operator in _VERSION_PROPERTIES_REPR_OPERATORS:
            ...
            if version_ is not None:
                version_specifiers.append(
                    repr_operator + '.'.join(
                        str(item)
                        for item in _version_as_tuple(version_)
                    )
                )
