...
def calling_function_qualified_name(depth=1):
    ...
    if not isinstance(depth, int):
        ...
        raise TypeError(
            'The parameter `depth` for `sob.utilities.calling_function_qualified_name` must be an `int`, not' +
            (
...
def get_io_url(file_like_object):
    # type: (IOBase) -> Optional[str]
    """
    Get the URL from which an input-outpuy (file-like) object was sourced
    """
